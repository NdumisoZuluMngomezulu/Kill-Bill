...
public class MultiClientHandler {
    ...
    static public Map<String, SingleClientHandler> activeClients = new ConcurrentHashMap<>();
     ...
     */
    public MultiClientHandler(List<Socket> socketList) {
    ...
    private void handleCommand(String userInput) {
        ...
        switch (userInput) {
            case "view":
                ...
                break;
            case "robots":
                if ((SingleClientHandler.robotObjects.isEmpty())) {
                ...
                break;
            case "dump":
                Menu.displayObstaclesAndRobots();
                break;
            case "quit":
    ...
    public void acceptClients(ServerSocket serverSocket) {
        while (true) {
            try {
                ...
                SimpleServerHandler TaskForEachClient = new SimpleServerHandler(socket, this, client);
    ...
    */
    ...
    public void routeShotNotification(String targetId, String shooterId) {
        SimpleServerHandler targetHandler = activeClients.get(targetId);
    ...
    public void createWorld(int x, int y) {
        ConfigWorld.setTopLeftX_world(0);
        ConfigWorld.setTopLeftY_world(0);
        ConfigWorld.setBottomRightX_world(x-1);
        ConfigWorld.setBottomRightY_world(y-1);
    }
